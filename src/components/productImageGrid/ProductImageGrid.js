function ProductImageGrid({ currentProductImage, handleClick, gridWidth }) {
  const smallImgStyle = (
    n
  ) => `w-full rounded-md hover:opacity-50 transition duration-500 ease-out 
    cursor-pointer ${currentProductImage === n ? " opacity-25" : ""}`;
  const smallImgContainerStyle = (n) =>
    `${currentProductImage === n ? "border-2 border-orange" : ""} rounded-lg `;
